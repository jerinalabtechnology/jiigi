...
const SearchLocationBox = ({ open, setIsModalOpen }) => {
  ...
  useEffect(() => {
    ...
    const fetchPlaces = () => {
      ...
      const request = {
        ...
        // types: ['geocode', 'establishment'], // Optional: restrict to specific types
      };
  ...
  }, [searchInput, autocompleteService, isSelecting, sessionToken]);
  ...
  const handlePlaceSelect = async (place) => {
    ...
    try {
      const fullAddress = await fetchPlaceDetails(place.place_id);

      setFullAddress({
        lat: fullAddress?.geometry?.location?.lat(), // Note: Google Maps JS API returns functions for lat/lng
  ...
  const handleSearchPlace = async () => {
    if (fullAddress.lat && fullAddress.lng && fullAddress.address) {
      ...
      // 🆕 Handle case where user typed but didn't select from dropdown
      setIsLoading(true);
      ...
      autocompleteService.getPlacePredictions({ input: searchInput, sessionToken: sessionToken }, (predictions, status) => {
          if (status === window.google.maps.places.PlacesServiceStatus.OK && predictions && predictions.length > 0) {
              const firstPlace = predictions[0];
              
              fetchPlaceDetails(firstPlace.place_id).then(details => {
                 if (details) {
      ...
      // If we only have the address (manual entry), still allow proceeding
      dispatch(locationAddressData(fullAddress.address));
      router.push("/");
    } else {
      ...
      <div className="search_input_box w-full relative flex items-center card_bg p-2 rounded-md mt-4 sm:mt-6 lg:mt-10 gap-4">
        ...
        <div className="flex items-center justify-center gap-2 sm:gap-4 relative">
          ...
          <button
            ...
            disabled={!searchInput.trim() && (!fullAddress.lat || !fullAddress.lng || !fullAddress.address)}
          >
            <IoSearch size={18} />
