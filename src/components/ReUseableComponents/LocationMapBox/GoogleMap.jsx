...
const Map = ({
  ...
  const markerPosition = useMemo(() => {
    ...
    return { lat: 23.2387, lng: 69.67 }; // Default Bhuj
  }, [latitude, longitude]);
  ...
  const handlePlaceSelect = async (place) => {
    ...
    try {
      ...
      if (!placeDetails) {
        console.error("No place details found. Response:", placeDetailsRes);
        toast.error(t("errorFetchingPlaceDetails") || "Error fetching place details");
        return;
      }
        ...
        <div className="absolute bottom-3 left-0 right-0 px-4 z-10 block">
          ...
          >
            {visibleProviders.map((provider) => (
              ...
              >
                <Link href={`/provider-details/${provider?.slug}`} title={provider?.company_name}>
